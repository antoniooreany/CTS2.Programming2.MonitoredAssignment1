...
public class PixelPane extends Application {
    ...
    private static final double RIGHT_INSET = 100;
	...
	public void start(Stage myStage) {
		...
		grid.setBorder(new Border(new BorderStroke(Color.GRAY, BorderStrokeStyle.SOLID, CornerRadii.EMPTY,
		...
		grid.addEventHandler(MouseEvent.MOUSE_PRESSED, new EventHandler<MouseEvent>(){
			public void handle(MouseEvent me) {
				if (me.isPrimaryButtonDown()) {
					int x = (int) ((me.getX()-10)/(RHEIGHT+HGAP));
					int y = (int) ((me.getY()-10)/(RWIDTH+VGAP));
